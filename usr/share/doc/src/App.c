...
int difficulty = HARD; // Default difficulty
#define AI_PLAYER 2
...
void draw_thick_circle(SDL_Renderer *renderer, int centerX, int centerY,
...
bool initSDL() {
  ...
  surface = SDL_LoadBMP("../assets/image/t.bmp");
  ...
  surface = SDL_LoadBMP("../assets/image/welcome_message.bmp");
...
int playagain(GameState *game) {
  SDL_Window *newwin = SDL_CreateWindow("Would you like to restart the game?",
                                        SDL_WINDOWPOS_CENTERED,
                                        SDL_WINDOWPOS_CENTERED, 340, 340, 0);
  ...
  SDL_Renderer *renderer =
      SDL_CreateRenderer(newwin, -1, SDL_RENDERER_ACCELERATED);
  if (!renderer) {
  ...
  SDL_Surface *image = SDL_LoadBMP("../assets/image/yes.bmp");
  if (!image) {
  ...
  while (SDL_WaitEvent(&event)) {
    ...
    if (event.type == SDL_KEYDOWN) {
      ...
      if (x >= yes.x && x <= yes.x + yes.w && y >= yes.y &&
          y <= yes.y + yes.h) {
        goto yes;
      }
...
void handleMouseClick(GameState *gameState, int x, int y) {
  ...
  // Human clicks only valid if it's their turn
  if (gameState->currentPlayer != HUMAN_PLAYER)
    return;

  if (gameState->board[row][col] == 0) { // Only allow moves in empty cells
    // Human makes move
    gameState->board[row][col] = HUMAN_PLAYER;
    renderGame(gameState);

    if (WinorNot(gameState)) {
      surface = (gameState->currentPlayer != HUMAN_PLAYER)
                    ? SDL_LoadBMP("../assets/image/AI_Win.bmp.")
                    : SDL_LoadBMP("../assets/image/X_win.bmp");
      ...
      return;
    } else if (isfull(*gameState)) {
      SDL_Delay(1000);
      surface = SDL_LoadBMP("../assets/image/tic_tac_toe_draw.bmp");
      texture = SDL_CreateTextureFromSurface(renderer, surface);
      SDL_FreeSurface(surface);
      SDL_RenderClear(renderer);
    ...
    // if (WinorNot(gameState)) {
    //   surface =
    //       SDL_LoadBMP("/home/darkemperor/aathi/my-learnig-path-/TIC_TAC_TOE/"
    //                   "usr/share/doc/../assets/image/tic_win.bmp");
    //   texture = SDL_CreateTextureFromSurface(renderer, surface);
    //   SDL_FreeSurface(surface);
    //   SDL_RenderClear(renderer);
    //   SDL_RenderCopy(renderer, texture, NULL, &imager);
    ...
    //   surface =
    //       SDL_LoadBMP("/home/darkemperor/aathi/my-learnig-path-/TIC_TAC_TOE/"
    //                   "usr/share/doc/../assets/image/tic_tac_toe_draw.bmp");
    //   texture = SDL_CreateTextureFromSurface(renderer, surface);
    //   SDL_FreeSurface(surface);
    //   SDL_RenderClear(renderer);
    //   SDL_RenderCopy(renderer, texture, NULL, NULL);
...
int miniMax(GameState *gamestate, int depth, int isMax) {
  ...
  if (score == 10)
    return score - depth; // Favor quick wins
  if (score == -10)
    return score + depth; // Delay opponent win
  if (isfull(*gamestate))
    return 0; // Draw
...
int evaluate(GameState game) {
  ...
  for (int row = 0; row < 3; row++) {
    if (game.board[row][0] == game.board[row][1] &&
      ...
      if (game.board[row][0] == AI_PLAYER)
        return +10;
      else if (game.board[row][0] == HUMAN_PLAYER)
        return -10;
    }
  }
  ...
  for (int col = 0; col < 3; col++) {
    if (game.board[0][col] == game.board[1][col] &&
      ...
      if (game.board[0][col] == AI_PLAYER)
        return +10;
      else if (game.board[0][col] == HUMAN_PLAYER)
        return -10;
    }
  }
  ...
  if (game.board[0][0] == game.board[1][1] &&
    ...
    if (game.board[0][0] == AI_PLAYER)
      return +10;
    else if (game.board[0][0] == HUMAN_PLAYER)
      return -10;
  }
  if (game.board[0][2] == game.board[1][1] &&
      game.board[1][1] == game.board[2][0]) {
    if (game.board[0][2] == AI_PLAYER)
      return +10;
    else if (game.board[0][2] == HUMAN_PLAYER)
      return -10;
  }
...
float get_score(GameState *GameState, int row, int col) {
  ...
  if (row > 0 && GameState->board[row - 1][col] == player)
    scr++; // Top
  if (row < 2 && GameState->board[row + 1][col] == player)
    scr++; // Bottom
  if (col > 0 && GameState->board[row][col - 1] == player)
    scr++; // Left
  if (col < 2 && GameState->board[row][col + 1] == player)
    scr++; // Right

  if (row > 0 && col > 0 && GameState->board[row - 1][col - 1] == player)
    scr++; // Top-left
  if (row > 0 && col < 2 && GameState->board[row - 1][col + 1] == player)
    scr++; // Top-right
  if (row < 2 && col > 0 && GameState->board[row + 1][col - 1] == player)
    scr++; // Bottom-left
  if (row < 2 && col < 2 && GameState->board[row + 1][col + 1] == player)
    scr++; // Bottom-right
